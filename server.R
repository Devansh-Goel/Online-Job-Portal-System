...
shinyServer(function(input, output, session) {
    ...
    plotTitle <- reactive({
        ...
        if(input$userName == "") {
            ...
            paste(input$userName, "'s Job ", sep = "")
        }
    ...
    template <- reactive({
        if( nrow(visNode()) == 1 ) {
            ""
        } else if ( nrow(visNode()) == 1 ) {
            png::readPNG("./www/Formpage_1.png")
        } else if ( nrow(visNode()) == 2 ) {
            png::readPNG("./www/Formpage_2.png")
        }  
    })
    ...
    plotInput <- reactive({
        ...
        if(input$returnpdf){
            ...
            pdf("./www/Report.pdf", width=as.numeric(8), height=as.numeric(11))
